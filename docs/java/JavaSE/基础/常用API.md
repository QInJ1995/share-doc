...
Java官方其实已经给 写好了很多很多类，每一个类中又提供了一系列方法来解决与这个类相关的问题。
...
除了Java官方提供的API，还一些第三方的公司或者组织也会提供一些API，甚至比Java官方提供的API更好用，在需要的时候 也会告诉大家怎么用。
...
- 如果当前程序中，要调用Java.lang包下的程序，不需要 导包的，可以直接使用。
...
接下来 学习String这个类，也就是学对字符串进行处理。为什么要学习字符串处理呢？因为在开发中对于字符串的处理还是非常常见的。
...
关于String类是用来干什么的，以及String类对象的创建 就学习到这里。最后总结一下
...
在上一节课中， 学习了如何通过字符串对象封装数据，接下来 学习调用String类的方法对象字符串数据进行处理。
...
下面 就把String类中的方法，按照方法的调用规则，先快速过一遍。（注意：第一次调用API方法，都是看着API方法来调用用的，不是背的）
...
在上一节， 学习了字符串的一些常用方法，在实际工作中用这些方法解决字符串的常见问题是完全足够的，但是在面试时可能会问一些原理性的东西。
...
关于String的案例， 先练习到这里。以后遇到对字符串进行操作的需求，优先找String类有没有提供对应的方法。
...
![1662620084702](/image/java/JavaSE/基础/常用API/1662620084702.png)
...
![1662620389155](/image/java/JavaSE/基础/常用API/1662620389155.png)
...
![1662622656784](/image/java/JavaSE/基础/常用API/1662622656784.png)
...
![1662623052476](/image/java/JavaSE/基础/常用API/1662623052476.png)

![1662623321970](/image/java/JavaSE/基础/常用API/1662623321970.png)

![1662623468659](/image/java/JavaSE/基础/常用API/1662623468659.png)

![1662623624269](/image/java/JavaSE/基础/常用API/1662623624269.png)
...
上一个ArrayList应用案例中， 往集合存储的元素是String类型的元素，实际上在工作中 经常往集合中自定义存储对象。
...
1. 在外卖系统中，每一份菜都包含，菜品的名称、菜品的原价、菜品的优惠价、菜品的其他信息。那 就可以定义一个菜品类（Food类），用来描述每一个菜品对象要封装那些数据。
2. 接着再写一个菜品管理类（FoodManager类），提供展示操作界面、上架菜品、浏览菜品的功能。

- 首先 先定义一个菜品类（Food类），用来描述每一个菜品对象要封装那些数据。
