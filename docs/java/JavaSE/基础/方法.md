...
本节 学习的内容是方法。方法也是Java语言中一个很重要的组成部分，在实际开发中几乎每时每刻都在使用方法。所以对于今天的课程一定要搞清楚。
...
![1661667297650](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661667297650.png)

看一个需求，比如现在张工、李工两个人都需要求两个整数的和。不使用方法，代码如下。
...
阅读上面的代码， 不难发现。两次求和的代码中，除了求和的数据不一样，代码的组织结构完全一样。
...
![1661668878007](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661668878007.png)

学习完方法的执行流程之后，下面有几个注意事项需要 写代码时注意一下。

**1.3 定义方法的注意点**

![1661680196574](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661680196574.png)
...
![1661680445407](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661680445407.png)

那么让程序的逻辑更加清晰，是如何体现的呢？ 比如， 后期会用所学习的技术，做一个ATM系统，ATM系统中有查看账户、存钱、取钱、修改密码等功能，到时候 可以把每一个功能都写成一个方法。如下图所示，这样程序的逻辑就更加清晰了。

![1661680833652](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661680833652.png)
...
刚才 学习了定义完整格式的方法。但是实际开发中，需要按照方法解决的实际业务需求，设计出合理的方法形式来解决问题。

实际上设计一个合理的方法，需要重点关注下面两点

![1661685360525](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661685360525.png)

![1661685287374](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661685287374.png)
...
接下来 看几个案例代码，练习根据实际需求定义出合理的方法

**需求1：写一个方法，打印3个"Hello World"**

分析：需求已经非常明确，打印的是3个HelloWorld，在方法中直接循环3次就可以完成需求。不需要外部给方法传递数据，所以不需要参数。

![1661686972979](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661686972979.png)
...
![1661687241729](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661687241729.png)

## 三、方法使用常见的问题

自己第一次写方法时，或多或少会可能会出现一些问题。下面把使用方法时，常见的问题整理一下。

目的是，以后写方法时避免出现这些问题。一旦出现这些问题，要知道是什么原因。
...
![1661687914420](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661687914420.png)
...
![1661687941843](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661687941843.png)
...
刚才 已经写了好几个方法并成功调用了。但是不知道奇不奇怪一个问题。方法在计算机的内存中到底是怎么干的呢？

为了让大家更加深刻的理解方法的执行过程，接下来，讲一下方法在计算机中的执行原理。理解方法的执行原理，对 以后知识的学习也是有帮助的。

知道Java程序的运行，都是在内存中执行的，而内存区域又分为栈、堆和方法区。那Java的方法是在哪个内存区域中执行呢？

答案是栈内存。 **每次调用方法，方法都会进栈执行；执行完后，又会弹栈出去。**

方法进栈和弹栈的过程，就类似于手枪子弹夹，上子弹和击发子弹的过程。最后上的一颗子弹是，第一个打出来的；第一颗上的子弹，是最后一个打出来的。

![1661689511649](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661689511649.png)
...
![1661692070922](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661692070922.png)

### 5.1 有返回值的方法，内存分析

下面 分析一下，求两个整数和的代码，在内存中的执行原理。
...
![1661694127049](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661694127049.png)

### 5.2 无返回值的方法，内存分析

刚才 分析的是有有参数有返回值的方法内存原理。下面再分析一个无返回值、无参数的内存原理。
...
![1661696067585](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661696067585.png)
...
刚才 学习了方法运行的原理，相信大家对方法的运行过程有更加深刻的认识。但是方法参数的传递过程还需要，还需要进一步学习一下。

因为 刚才演示的一些方法中传递的参数都是基本类型，实际上参数还可以是传递引用类型。接下来，学习一下当参数是基本类型时、和参数是引用类型时的区别。

先记住一个结论：**Java的参数传递机制都是：值传递**

所谓值传递：指的是在传递实参给方法的形参的时候，传递的是实参变量中存储的值的副本。 肯定想知道，形参是什么？实参又是什么呢？ 请看下面这个张图

![1661725157681](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661725157681.png)

### 6.1 参数传递的基本类型数据

接下来，看一下方法参数传递是基本类型数据时，内存中是怎么执行的。

![1661725470322](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661725470322.png)

把参数传递的结论再复习一下：**Java的参数传递机制都是：值传递，传递的是实参存储的值的副本。**

### 6.3 参数传递的是引用数据类型

接下来，看一下方法的参数是引用类型的数据时，内存中是怎么执行的。

![1661728059814](https://lsky-pro.qinlaoshi.dpdns.org:2083/qls/1661728059814.png)

发现调用change方法时参数是引用类型，**实际上也是值传递，只不过参数传递存储的地址值**。此时change方法和main方法中两个方法中各自有一个变量arrs，这两个变量记录的是同一个地址值[I@4c873330，change方法把数组中的元素改了，main方法在访问时，元素已经被修改了。
...
认识了方法重载，那么方法重载有哪些应用场景呢？

一般在开发中， 经常需要为处理一类业务，提供多种解决方案，此时用方法重载来设计是很专业的。
...
所以 可以设计几个名称相同的方法，这样调用者调用起来就不用记那么多名字了
...
3、方法重载有啥应用场景？
 答：开发中 经常需要为处理一类业务，提供多种解决方案，此时用方法重载来设计是很 专业的。
```

## 九、return单独使用

关于方法的定义， 还剩下最后一种特殊用法，就是在方法中单独使用return语句，可以用来提前结束方法的执行。
