...
前面我们学习了IO流，我们知道IO流是用来读、写文件中的数据。但是我们接触到的文件都是普通的文本文件，普通的文本文件里面的数据是没有任何格式规范的，用户可以随意编写，如下图所示。

像这种普通的文本文件，没有任何规律可言，不方便程序对文件中的数据信息处理。

![1667990938340](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667990938340.png)

在以后的Java开发过程中还会遇到一些特殊的文本文件，这些文件是有一些格式要求的，方便程序对文件中的数据进行处理。

比如，后面我们会用到两种特殊的文本文件，一种是properties文件，还有一种是xml文件。如下图所示。

![1667991153379](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667991153379.png)
...
![1667991441046](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667991441046.png)
...
![1667992083258](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667992083258.png)

接下来，我们学习如何读取属性文件中的数据。这里需要介绍一个叫Properties.
...
![1667992486134](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667992486134.png)
...
![1667993320872](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667993320872.png)
...
![1667993682237](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667993682237.png)
...
![1667993581146](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667993581146.png)

## 二、XML文件

在上一节我们学习了properties属性文件。接下来我们再学习一种在开发中经常使用的文件，叫做xml文件。我们先来介绍一下，什么是xml文件，然后再来学习如何读取xml文件中的数据。
...
![1667993965682](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667993965682.png)

**接下，可以跟着步骤新建一个XML文件，试试！**

![1667994441934](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667994441934.png)
...
![1667995490654](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667995490654.png)
...
![1667996374837](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667996374837.png)

由于DOM4J是第三方提供的，所以需要把第三方提供的Jar包导入到自己的项目中来，才可以使用。具体步骤如下：

![1667996538290](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667996538290.png)
...
![1667996650787](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667996650787.png)

DOM4J解析XML需要用到的方法如下图所示

![1667996750188](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667996750188.png)
...
![1667996995326](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667996995326.png)

### 2.3 XML解析2

获取到XML文件的根元素之后，接下来，就可以用根元素在获取到它里面的子元素（包括子标签、表属性等）。需要用到的方法如下图所示

![1667997211327](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1667997211327.png)
...
各位小伙伴，关于XML还有最后一个知识需要大家了解一下。这个知识叫做约束XML文件的编写，我讲这个知识的目的是因为以后在开发过程中会遇到这个知识，但是这个代码不需要大家写，了解一下就可以了。
...
![1668001422123](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668001422123.png)
...
- DTD约束案例
  ...
  ![1668001621567](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668001621567.png)
...
- Schame约束案例
  ...
  ![1668001745089](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668001745089.png)

## 三、日志技术

### 3.1 日志概述

接下来我们学习一个将来我们会用到的一个技术，叫做日志技术。首先肯定想知道什么是日志？
...
![1668044513873](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668044513873.png)

**这里推荐使用Logback日志框架，也在行业中最为广泛使用的。**

**Logback日志分为哪几个模块**

![1668044711404](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668044711404.png)
...
![1668045391911](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668045391911.png)

同时在文件中，也有一份这样的日志信息。文件在哪里内，从配置文件中去找

![1668045471304](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668045471304.png)

打开`D:/log/test-data.log`看一下文件中是否有记录日志吧！！

![1668045530645](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668045530645.png)
...
![1668045955362](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668045955362.png)

- **如下图所示，控制打开和关闭日志**

![1668046078574](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668046078574.png)

- **如下图所示，控制日志的输出的格式**

  日志格式是由一些特殊的符号组成，可以根据需要删减不想看到的部分。比如不想看到线程名那就不要[%thread]。但是不建议更改这些格式，因为这些都是日志很基本的信息。

![1668046216355](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668046216355.png)

### 3.5 配置日志级别

![1668046420402](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668046420402.png)

- 在哪里配置日志级别呢？如下图所示

![1668046551345](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668046551345.png)
...
接下来我们来学习一个全新而且非常重要的知识，叫做多线程。首先聊聊什么是线程？**线程其实是程序中的一条执行路径。**

 我们之前写过的程序，其实都是单线程程序，如下图代码，如果前面的for循环没有执行完，for循环下面的代码是不会执行的。

![1668046984412](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668046984412.png)
...
![1668047091631](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668047091631.png)
...
![1668047848218](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668047848218.png)

**最后我们还需要注意一点**：不能直接去调用run方法，如果直接调用run方法就不认为是一条线程启动了，而是把Thread当做一个普通对象，此时run方法中的执行的代码会成为主线程的一部分。此时执行结果是这样的。

![1668048108548](/image/java/JavaSE/进阶/特殊文件、日志技术、多线程/1668048108548.png)
