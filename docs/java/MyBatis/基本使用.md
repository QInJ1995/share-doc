...
* 编写测试用例
  ...
  ![项目结构](/image/java/MyBatis/基本使用/image-20210729112907106.png)
...
* 安装 MyBatisX 插件
  ...
  * 安装方式
    ...
    ![安装界面](/image/java/MyBatis/基本使用/image-20210729113304743.png)

    > 注意：安装完毕后需要重启IDEA

  * 插件效果

    ![插件效果](/image/java/MyBatis/基本使用/image-20210729164450524.png)
    ...
    ![自动生成](/image/java/MyBatis/基本使用/image-20210729165337223.png)

### 1.2  查询所有数据

![查询所有数据](/image/java/MyBatis/基本使用/image-20210729165724838.png)
...
* 编写接口方法：Mapper接口
  ...
  * 参数：无
    ...
    ![无参数](/image/java/MyBatis/基本使用/image-20210729171208737.png)

  * 结果：`List<Brand>`

    我们会将查询出来的每一条数据封装成一个 `Brand` 对象，而多条数据封装多个 `Brand` 对象，需要将这些对象封装到List集合中返回。

    ![结果封装](/image/java/MyBatis/基本使用/image-20210729171146911.png)
...
![测试结果](/image/java/MyBatis/基本使用/image-20210729172544230.png)
...
![字段对比](/image/java/MyBatis/基本使用/image-20210729173210433.png)
...
![查询详情](/image/java/MyBatis/基本使用/image-20210729180118287.png)
...
* 编写接口方法：Mapper接口
  ...
  ![接口方法](/image/java/MyBatis/基本使用/image-20210729180604529.png)
...
* 编写SQL语句：SQL映射文件
  ...
  ![SQL映射文件](/image/java/MyBatis/基本使用/image-20210729180709318.png)
...
![测试结果](/image/java/MyBatis/基本使用/image-20210729182223137.png)
...
* ${} ：拼接SQL。底层使用的是 `Statement`，会存在SQL注入问题。如下图将 映射配置文件中的 #{} 替换成 ${} 来看效果
  ...
  ![SQL注入问题](/image/java/MyBatis/基本使用/image-20210729184156019.png)
...
![特殊字段处理](/image/java/MyBatis/基本使用/image-20210729184756094.png)
...
* 转义字符
  ...
  ![转义字符](/image/java/MyBatis/基本使用/image-20210729185128686.png)

* <![CDATA[内容]]>

  ![CDATA转义](/image/java/MyBatis/基本使用/image-20210729185030318.png)

### 1.4  多条件查询

![多条件查询](/image/java/MyBatis/基本使用/image-20210729203804276.png)
...
![条件表达式](/image/java/MyBatis/基本使用/image-20210729204458815.png)
...
* if 标签：条件判断
  ...
  * test 属性：逻辑表达式
  ...
  ![动态SQL结果](/image/java/MyBatis/基本使用/image-20210729212510291.png)
...
![单个条件动态SQL](/image/java/MyBatis/基本使用/image-20210729213613029.png)
...
![测试结果](/image/java/MyBatis/基本使用/image-20210729214548756.png)

### 1.6  添加数据

![添加数据](/image/java/MyBatis/基本使用/image-20210729214917317.png)

如上图是我们平时在添加数据时展示的页面，而我们在该页面输入想要的数据后添加 `提交` 按钮，就会将这些数据添加到数据库中。接下来我们就来实现添加数据的操作。

* 编写接口方法

  ![接口方法](/image/java/MyBatis/基本使用/image-20210729215351651.png)

  参数：除了id之外的所有的数据。id对应的是表中主键值，而主键我们是 ==自动增长== 生成的。

* 编写SQL语句

  ![SQL语句](/image/java/MyBatis/基本使用/image-20210729215537167.png)
...
![执行结果](/image/java/MyBatis/基本使用/image-20210729220348255.png)
...
![京东订单](/image/java/MyBatis/基本使用/image-20210729221207962.png)

订单数据存储在订单表中，订单项存储在订单项表中。

* 添加订单数据

  ![添加订单数据](/image/java/MyBatis/基本使用/image-20210729221049462.png)

* 添加订单项数据，订单项中需要设置所属订单的id

  ![添加订单项数据](/image/java/MyBatis/基本使用/image-20210729221058898.png)
...
![修改数据](/image/java/MyBatis/基本使用/image-20210729222642700.png)
...
![测试结果](/image/java/MyBatis/基本使用/image-20210729224205522.png)

从结果中SQL语句可以看出，只修改了 `status`  字段值，因为我们给的数据中只给Brand实体对象的 `status` 属性设置值了。这就是 `set` 标签的作用。

### 1.8  删除一行数据

![删除一行数据](/image/java/MyBatis/基本使用/image-20210729224549305.png)
...
![批量删除](/image/java/MyBatis/基本使用/image-20210729225713894.png)
...
* 运行代码结果如下
  ...
  ![运行结果](/image/java/MyBatis/基本使用/image-20210805230303461.png)
...
* 运行程序则可以看到错误
  ...
  ![运行错误](/image/java/MyBatis/基本使用/image-20210805231727206.png)
...
* 将之前案例中 `UserMapper.xml` 中的 根据id查询数据 的 `statement` 注释掉
  ...
  ![注释statement](/image/java/MyBatis/基本使用/image-20210805235229938.png)

* 在 `UserMapper` 接口的 `selectById` 方法上添加注解

  ![添加注解](/image/java/MyBatis/基本使用/image-20210805235405070.png)
...
![官方文档说明](/image/java/MyBatis/基本使用/image-20210805234302849.png)

所以，==注解完成简单功能，配置文件完成复杂功能。==

而我们之前写的动态 SQL 就是复杂的功能，如果用注解使用的话，就需要使用到 Mybatis 提供的SQL构建器来完成，而对应的代码如下：

![SQL构建器](/image/java/MyBatis/基本使用/image-20210805234842497.png)
